    ...
    if (!add) {
      for (let shape in data) {
        if (data.hasOwnProperty(shape)) {
          ...
          let text =
            data[shape].name.toUpperCase() +
            "\n\n" +
            "Width: " +
            data[shape].getWidth +
            "\n" +
            "Height: " +
            data[shape].getHeight +
            "\n" +
            "Color: " +
            data[shape].getColour;
...
class MainController {
  editorTools = {
    ...
    colour_apply() {
      ...
      this.model.editShape(
        this.model.getCurrentSelectedShape,
        "colour",
        colour
      );
    },
  ...
  toolClick(event) {
    if (TOOLS.includes(event.target.id)) {
      if (EDITOR_TOOLS.includes(event.target.id)) {
        if (!this.model.getCurrentSelectedShape) return;
  ...
  mouseMovementHandler(event) {
    ...
    if (this.isDragging) {
      ...
      let left =
        window.innerWidth -
        this.view.canvasContainer.getBoundingClientRect().width;
  ...
  canvasClick() {
    if (
      ...
      this.model.getCurrentSelectedTool &&
      this.view.elem
    ) {
      if (!this.isDragging) {
